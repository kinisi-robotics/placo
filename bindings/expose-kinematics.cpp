...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(frametask_configure_overloads, configure,
                                       2, 4);
...
void exposeKinematics() {
  class_<KinematicsSolver> solver_class =
      class__<KinematicsSolver>("KinematicsSolver", init<RobotWrapper &>())
          ...
          .add_property(
              "robot",
              make_function(
                  +[](const KinematicsSolver &solver) -> model::RobotWrapper & {
                    return solver.robot;
                  },
                  return_value_policy<reference_existing_object>()))

          // Position and CoM task
          .def<PositionTask &(KinematicsSolver::*)(std::string,
                                                   Eigen::Vector3d)>(
              "add_position_task", &KinematicsSolver::add_position_task,
              return_internal_reference<>())
          .def<RelativePositionTask &(
              KinematicsSolver::*)(std::string, std::string, Eigen::Vector3d)>(
              "add_relative_position_task",
              &KinematicsSolver::add_relative_position_task,
              return_internal_reference<>())
          .def<CoMTask &(KinematicsSolver::*)(Eigen::Vector3d)>(
              "add_com_task", &KinematicsSolver::add_com_task,
              return_internal_reference<>())

          // Orientation task
          .def<OrientationTask &(KinematicsSolver::*)(std::string,
                                                      Eigen::Matrix3d)>(
              "add_orientation_task", &KinematicsSolver::add_orientation_task,
              return_internal_reference<>())
          .def<RelativeOrientationTask &(
              KinematicsSolver::*)(std::string, std::string, Eigen::Matrix3d)>(
              "add_relative_orientation_task",
              &KinematicsSolver::add_relative_orientation_task,
              return_internal_reference<>())

          // Frame task
          .def<FrameTask (KinematicsSolver::*)(std::string, Eigen::Affine3d)>(
              "add_frame_task", &KinematicsSolver::add_frame_task)
          .def<RelativeFrameTask (KinematicsSolver::*)(std::string, std::string,
                                                       Eigen::Affine3d)>(
              "add_relative_frame_task",
              &KinematicsSolver::add_relative_frame_task)

          // Axis align task
          .def<AxisAlignTask &(KinematicsSolver::*)(std::string,
                                                    Eigen::Vector3d,
                                                    Eigen::Vector3d)>(
          ...
          // Joint task
          .def<JointsTask &(KinematicsSolver::*)(void)>(
              "add_joints_task", &KinematicsSolver::add_joints_task,
              return_internal_reference<>())

          // Gear task
          .def<GearTask &(KinematicsSolver::*)(void)>(
              "add_gear_task", &KinematicsSolver::add_gear_task,
              return_internal_reference<>())

          // Wheel task
          .def<WheelTask &(KinematicsSolver::*)(std::string, double, bool)>(
              "add_wheel_task", &KinematicsSolver::add_wheel_task,
              return_internal_reference<>())

          // Distance task
          .def<DistanceTask &(KinematicsSolver::*)(std::string, std::string,
                                                   double)>(
              "add_distance_task", &KinematicsSolver::add_distance_task,
              return_internal_reference<>())

          // Centroidal Momentum Task
          .def<CentroidalMomentumTask &(KinematicsSolver::*)(Eigen::Vector3d)>(
              "add_centroidal_momentum_task",
          ...
          // Manipulability task
          .def<ManipulabilityTask &(KinematicsSolver::*)(std::string,
                                                         std::string, double)>(
              "add_manipulability_task",
              &KinematicsSolver::add_manipulability_task,
              return_internal_reference<>())
          ...
          .def("add_joint_space_half_spaces_constraint",
               &KinematicsSolver::add_joint_space_half_spaces_constraint,
               return_internal_reference<>())

          // Cone constraint
          .def<ConeConstraint &(KinematicsSolver::*)(std::string, std::string,
                                                     double)>(
              "add_cone_constraint", &KinematicsSolver::add_cone_constraint,
              return_internal_reference<>())

          // Yaw constraint
          .def<YawConstraint &(KinematicsSolver::*)(std::string, std::string,
                                                    double)>(
              "add_yaw_constraint", &KinematicsSolver::add_yaw_constraint,
              return_internal_reference<>())

          // Distance constraint
          .def<DistanceConstraint &(KinematicsSolver::*)(std::string,
                                                         std::string, double)>(
              "add_distance_constraint",
              &KinematicsSolver::add_distance_constraint,
              return_internal_reference<>())
          ...
          .def<void (KinematicsSolver::*)(Task &)>("add_task",
                                                   &KinematicsSolver::add_task)
          .def<void (KinematicsSolver::*)(Constraint &)>(
              "add_constraint", &KinematicsSolver::add_constraint)
          .def<void (KinematicsSolver::*)(Task &)>(
              "remove_task", &KinematicsSolver::remove_task)
          .def<void (KinematicsSolver::*)(FrameTask &)>(
              "remove_task", &KinematicsSolver::remove_task)
  ...
  class__<PositionTask, bases<Task>>(
      "PositionTask", init<RobotWrapper::FrameIndex, Eigen::Vector3d>())
      .add_property("frame_index", &PositionTask::frame_index)
      .add_property(
          "target_world",
          +[](const PositionTask &task) { return task.target_world; },
          &PositionTask::target_world)
      .add_property("mask", &PositionTask::mask, &PositionTask::mask);

  class__<RelativePositionTask, bases<Task>>(
      "RelativePositionTask", init<RobotWrapper::FrameIndex,
                                   RobotWrapper::FrameIndex, Eigen::Vector3d>())
      ...
      .add_property("mask", &RelativePositionTask::mask,
                    &RelativePositionTask::mask);

  class__<CoMTask, bases<Task>>("CoMTask", init<Eigen::Vector3d>())
      .add_property(
          "target_world",
          +[](const CoMTask &task) { return task.target_world; },
          &CoMTask::target_world)
      .add_property("mask", &CoMTask::mask, &CoMTask::mask);

  class__<OrientationTask, bases<Task>>(
      "OrientationTask", init<RobotWrapper::FrameIndex, Eigen::Matrix3d>())
  ...
  class__<RelativeOrientationTask, bases<Task>>(
      ...
      init<RobotWrapper::FrameIndex, RobotWrapper::FrameIndex,
           Eigen::Matrix3d>())
      ...
      .add_property("mask", &RelativeOrientationTask::mask,
                    &RelativeOrientationTask::mask);

  class__<FrameTask>("FrameTask", init<>())
      .def(
          "position",
          +[](const FrameTask &task) -> PositionTask & {
      ...
      .def(
          "orientation",
          +[](const FrameTask &task) -> OrientationTask & {
            return *task.orientation;
          },
          return_internal_reference<>())
      .def("configure", &FrameTask::configure, frametask_configure_overloads())
      .add_property("T_world_frame", &FrameTask::get_T_world_frame,
                    &FrameTask::set_T_world_frame);

  class__<RelativeFrameTask>(
      "RelativeFrameTask",
      init<RelativePositionTask &, RelativeOrientationTask &>())
      .def(
          "position",
          +[](const RelativeFrameTask &task) -> RelativePositionTask & {
            return task.position;
          },
          return_internal_reference<>())
      ...
      .def(
          "orientation",
          +[](const RelativeFrameTask &task) -> RelativeOrientationTask & {
            return task.orientation;
          },
          return_internal_reference<>())
      .def("configure", &RelativeFrameTask::configure,
           frametask_configure_overloads())
      .add_property("T_a_b", &RelativeFrameTask::get_T_a_b,
                    &RelativeFrameTask::set_T_a_b);

  class__<AxisAlignTask, bases<Task>>(
      "AxisAlignTask",
      init<RobotWrapper::FrameIndex, Eigen::Vector3d, Eigen::Vector3d>())
      .add_property("frame_index", &AxisAlignTask::frame_index)
      .add_property(
          "axis_frame",
          +[](const AxisAlignTask &task) { return task.axis_frame; },
          &AxisAlignTask::axis_frame)
  ...
  class__<WheelTask, bases<Task>>("WheelTask",
                                  init<std::string, double, bool>())
      .add_property("joint", &WheelTask::joint)
      .add_property("radius", &WheelTask::radius)
      .add_property("omniwheel", &WheelTask::omniwheel)
      .add_property("T_world_surface",
                    make_getter(&WheelTask::T_world_surface,
                                return_value_policy<return_by_value>()),
                    make_setter(&WheelTask::T_world_surface));

  class__<DistanceTask, bases<Task>>(
      "DistanceTask",
      init<RobotWrapper::FrameIndex, RobotWrapper::FrameIndex, double>())
      .add_property("frame_a", &DistanceTask::frame_a)
      .add_property("frame_b", &DistanceTask::frame_b)
      .add_property("distance", &DistanceTask::distance,
                    &DistanceTask::distance);

  class__<CentroidalMomentumTask, bases<Task>>("CentroidalMomentumTask",
                                               init<Eigen::Vector3d>())
      .add_property("mask", &CentroidalMomentumTask::mask)
      .add_property("L_world", &CentroidalMomentumTask::L_world,
  ...
  class__<ManipulabilityTask, bases<Task>>(
      "ManipulabilityTask",
  ...
  class__<KineticEnergyRegularizationTask, bases<RegularizationTask>>(
      "KineticEnergyRegularizationTask");

  class__<Constraint, bases<tools::Prioritized>, boost::noncopyable>(
      "KinematicsConstraint", no_init);

  class__<AvoidSelfCollisionsConstraint, bases<Constraint>>(
      "AvoidSelfCollisionsKinematicsConstraint", init<>())
      .def_readwrite("self_collisions_margin",
                     &AvoidSelfCollisionsConstraint::self_collisions_margin)
      .def_readwrite("self_collisions_trigger",
                     &AvoidSelfCollisionsConstraint::self_collisions_trigger);

  class__<CoMPolygonConstraint, bases<Constraint>>(
      "CoMPolygonConstraint", init<std::vector<Eigen::Vector2d>, double>())
  ...
  class__<JointSpaceHalfSpacesConstraint, bases<Constraint>>(
  ...
  class__<ConeConstraint, bases<Constraint>>(
      "ConeConstraint", init<model::RobotWrapper::FrameIndex,
                             model::RobotWrapper::FrameIndex, double>())
  ...
  class__<YawConstraint, bases<Constraint>>(
      "YawConstraint", init<model::RobotWrapper::FrameIndex,
                            model::RobotWrapper::FrameIndex, double>())
      .def_readwrite("angle_max", &YawConstraint::angle_max);

  class__<DistanceConstraint, bases<Constraint>>(
      "DistanceConstraint", init<model::RobotWrapper::FrameIndex,
                                 model::RobotWrapper::FrameIndex, double>())
