...
HumanoidRobot::HumanoidRobot(std::string model_directory, int flags,
  ...
  if (model.existFrame("trunk") && model.existFrame("left_hip_yaw")) {
    dist_y_trunk_foot =
        fabs(get_T_a_b("trunk", "left_hip_yaw").translation().y());
  } else {
    std::cerr << "WARNING: Can't find trunk frames in the model, can't measure "
                 "leg spacing"
              << std::endl;
  }

  if (model.existFrame("head_base") && model.existFrame("head_pitch") &&
      model.existFrame("camera")) {
    dist_z_pan_tilt = get_T_a_b("head_base", "head_pitch").translation().z();
    dist_z_pan_camera = get_T_a_b("head_base", "camera").translation().z();
  } else {
    std::cerr << "WARNING: Can't find head frames in the model, camera_look_at "
                 "won't work"
              << std::endl;
  }
}

void HumanoidRobot::initialize() { init_config(); }
...
Eigen::Vector3d HumanoidRobot::get_com_velocity(Side support, Eigen::Vector3d omega_b)
{
  ...
  return J_u_C * J_u_inv * M + (J_a_C - J_u_C * J_u_inv * J_a) *
                                   state.qd.block(6, 0, model.nv - 6, 1);
}

Eigen::VectorXd HumanoidRobot::get_torques(Eigen::VectorXd acc_a,
  ...
  for (int i = 0; i < 8; i++) {
    if (i < 4) {
      J_c.col(i) = frame_jacobian("left_ps_" + std::to_string(i), "local")
                       .transpose()
                       .col(2);
    } else {
      J_c.col(i) = frame_jacobian("right_ps_" + std::to_string(i - 4), "local")
                       .transpose()
                       .col(2);
    }
  }
  ...
  Eigen::VectorXd h =
      use_non_linear_effects ? non_linear_effects() : generalized_gravity();
  Eigen::VectorXd h_bf = h.head(6);

  // Unactuated DoFs acceleration (floating base)
  Eigen::VectorXd acc_bf =
      M_bf.inverse() * ((J_c * contact_forces).head(6) - h_bf - M_a * acc_a);
...
Eigen::Vector2d HumanoidRobot::zmp(double omega,
                                   Eigen::Vector2d com_acceleration) {
  // ZMP = c - (1/omega^2) c_ddot
  return com_world().head(2) - (1 / pow(omega, 2)) * com_acceleration;
}

bool HumanoidRobot::camera_look_at(double &pan, double &tilt,
                                   const Eigen::Vector3d &P_world_target) {
  // Compute view vector in head yaw frame
  Eigen::Affine3d T_world_headBase = get_T_world_frame("head_base");
  Eigen::Vector3d P_headBase_target =
      T_world_headBase.inverse() * P_world_target;
  ...
  Eigen::Vector2d P_headPitchPlane_target(
      sqrt(pow(P_headBase_target.x(), 2) + pow(P_headBase_target.y(), 2)),
      P_headBase_target.z() - dist_z_pan_tilt);

  double theta = M_PI / 2 - atan2(P_headPitchPlane_target.y(),
                                  P_headPitchPlane_target.x());
...
void HumanoidRobot::read_from_histories(
    rhoban_utils::HistoryCollection &histories, double timestamp,
    std::string source, bool use_imu, Eigen::VectorXd qd_joints) {
  // Updating DOFs from replay
  for (const std::string &name : joint_names()) {
    set_joint(name,
              histories.number(source + ":" + name)->interpolate(timestamp));
    if (qd_joints.size() > 1) {
  ...
  double left_pressure =
      histories.number("left_pressure_0")->interpolate(timestamp) +
  ...
  double right_pressure =
      histories.number("right_pressure_0")->interpolate(timestamp) +
  ...
  if (use_imu) {
    ...
    Eigen::Matrix3d R_world_trunk =
        pinocchio::rpy::rpyToMatrix(imuRoll, imuPitch, imuYaw);
    update_from_imu(R_world_trunk);
  }

  // Setting the floating base velocity (TODO : check if it is correct)
  if (qd_joints.size() > 1) {
    Eigen::Vector3d omega_trunk =
        Eigen::Vector3d(histories.number("gyro_x")->interpolate(timestamp),
...
<<<<<<< HEAD
    J << frame_jacobian(support_frame(), pinocchio::ReferenceFrame::LOCAL)
             .topRows(3),
=======
    J << frame_jacobian(support_frame, pinocchio::ReferenceFrame::LOCAL).topRows(3),
>>>>>>> 81c9e0d7f65a6ad8ffc46d7f2f906caa4f947c70
