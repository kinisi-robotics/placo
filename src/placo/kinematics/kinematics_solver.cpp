...
KinematicsSolver::KinematicsSolver(model::RobotWrapper &robot_)
    : robot(robot_), masked_fbase(false) {
  N = robot.model.nv;
  problem.use_sparsity = false;
}

KinematicsSolver::~KinematicsSolver() { clear(); }

PositionTask &
KinematicsSolver::add_position_task(model::RobotWrapper::FrameIndex frame,
                                    Eigen::Vector3d target_world) {
  return add_task(new PositionTask(frame, target_world));
}

PositionTask &
KinematicsSolver::add_position_task(std::string frame,
                                    Eigen::Vector3d target_world) {
  return add_position_task(robot.get_frame_index(frame), target_world);
}

RelativePositionTask &KinematicsSolver::add_relative_position_task(
...
RelativePositionTask &KinematicsSolver::add_relative_position_task(
    std::string frame_a, std::string frame_b, Eigen::Vector3d target) {
  return add_relative_position_task(robot.get_frame_index(frame_a),
                                    robot.get_frame_index(frame_b), target);
}
...
OrientationTask &
KinematicsSolver::add_orientation_task(model::RobotWrapper::FrameIndex frame,
                                       Eigen::Matrix3d R_world_frame) {
  return add_task(new OrientationTask(frame, R_world_frame));
}

OrientationTask &
KinematicsSolver::add_orientation_task(std::string frame,
                                       Eigen::Matrix3d R_world_frame) {
  return add_orientation_task(robot.get_frame_index(frame), R_world_frame);
}

RelativeOrientationTask &KinematicsSolver::add_relative_orientation_task(
    model::RobotWrapper::FrameIndex frame_a,
...
RelativeOrientationTask &KinematicsSolver::add_relative_orientation_task(
    std::string frame_a, std::string frame_b, Eigen::Matrix3d R_a_b) {
  return add_relative_orientation_task(robot.get_frame_index(frame_a),
                                       robot.get_frame_index(frame_b), R_a_b);
}

FrameTask
KinematicsSolver::add_frame_task(model::RobotWrapper::FrameIndex frame,
                                 Eigen::Affine3d T_world_frame) {
  PositionTask &position =
      add_position_task(frame, T_world_frame.translation());
  OrientationTask &orientation =
      add_orientation_task(frame, T_world_frame.rotation());

  return FrameTask(&position, &orientation);
}

FrameTask KinematicsSolver::add_frame_task(std::string frame,
                                           Eigen::Affine3d T_world_frame) {
  return add_frame_task(robot.get_frame_index(frame), T_world_frame);
}

RelativeFrameTask KinematicsSolver::add_relative_frame_task(
    model::RobotWrapper::FrameIndex frame_a,
    model::RobotWrapper::FrameIndex frame_b, Eigen::Affine3d T_a_b) {
  RelativePositionTask &position =
      add_relative_position_task(frame_a, frame_b, T_a_b.translation());
  RelativeOrientationTask &orientation =
      add_relative_orientation_task(frame_a, frame_b, T_a_b.rotation());

  return RelativeFrameTask(position, orientation);
}

RelativeFrameTask KinematicsSolver::add_relative_frame_task(
    std::string frame_a, std::string frame_b, Eigen::Affine3d T_a_b) {
  return add_relative_frame_task(robot.get_frame_index(frame_a),
                                 robot.get_frame_index(frame_b), T_a_b);
}

AxisAlignTask &
KinematicsSolver::add_axisalign_task(model::RobotWrapper::FrameIndex frame,
...
AxisAlignTask &
KinematicsSolver::add_axisalign_task(std::string frame,
                                     Eigen::Vector3d axis_frame,
                                     Eigen::Vector3d target_axis_world) {
  return add_axisalign_task(robot.get_frame_index(frame), axis_frame,
                            target_axis_world);
}

JointsTask &
KinematicsSolver::add_joints_task(std::map<std::string, double> &joints) {
...
DistanceTask &
KinematicsSolver::add_distance_task(model::RobotWrapper::FrameIndex frame_a,
                                    model::RobotWrapper::FrameIndex frame_b,
                                    double distance) {
  return add_task(new DistanceTask(frame_a, frame_b, distance));
}

DistanceTask &KinematicsSolver::add_distance_task(std::string frame_a,
                                                  std::string frame_b,
                                                  double distance) {
  return add_distance_task(robot.get_frame_index(frame_a),
                           robot.get_frame_index(frame_b), distance);
}

CentroidalMomentumTask &
KinematicsSolver::add_centroidal_momentum_task(Eigen::Vector3d L_world) {
...
GearTask &KinematicsSolver::add_gear_task() { return add_task(new GearTask()); }

WheelTask &KinematicsSolver::add_wheel_task(std::string joint, double radius,
                                            bool omniwheel) {
...
ManipulabilityTask &
KinematicsSolver::add_manipulability_task(model::RobotWrapper::FrameIndex frame,
                                          ManipulabilityTask::Type type,
                                          double lambda) {
  return add_task(new ManipulabilityTask(frame, type, lambda));
}

ManipulabilityTask &KinematicsSolver::add_manipulability_task(std::string frame,
                                                              std::string type,
                                                              double lambda) {
...
KinematicsSolver::add_kinetic_energy_regularization_task(double magnitude) {
  KineticEnergyRegularizationTask &task =
      add_task(new KineticEnergyRegularizationTask());
  task.configure("kinetic_energy_regularization", Task::Priority::Soft,
                 magnitude);
...
CoMPolygonConstraint &KinematicsSolver::add_com_polygon_constraint(
...
KinematicsSolver::add_joint_space_half_spaces_constraint(Eigen::MatrixXd A,
                                                         Eigen::VectorXd b) {
  return add_constraint(new JointSpaceHalfSpacesConstraint(A, b));
}

ConeConstraint &
KinematicsSolver::add_cone_constraint(model::RobotWrapper::FrameIndex frame_a,
                                      model::RobotWrapper::FrameIndex frame_b,
                                      double angle_max) {
  return add_constraint(new ConeConstraint(frame_a, frame_b, angle_max));
}

ConeConstraint &KinematicsSolver::add_cone_constraint(std::string frame_a,
                                                      std::string frame_b,
                                                      double angle_max) {
  return add_cone_constraint(robot.get_frame_index(frame_a),
                             robot.get_frame_index(frame_b), angle_max);
}

YawConstraint &
KinematicsSolver::add_yaw_constraint(model::RobotWrapper::FrameIndex frame_a,
                                     model::RobotWrapper::FrameIndex frame_b,
                                     double angle_max) {
  return add_constraint(new YawConstraint(frame_a, frame_b, angle_max));
}

YawConstraint &KinematicsSolver::add_yaw_constraint(std::string frame_a,
                                                    std::string frame_b,
                                                    double angle_max) {
  return add_yaw_constraint(robot.get_frame_index(frame_a),
                            robot.get_frame_index(frame_b), angle_max);
}

DistanceConstraint &KinematicsSolver::add_distance_constraint(
    model::RobotWrapper::FrameIndex frame_a,
...
DistanceConstraint &KinematicsSolver::add_distance_constraint(
    std::string frame_a, std::string frame_b, double distance_max) {
  return add_distance_constraint(robot.get_frame_index(frame_a),
...
void KinematicsSolver::mask_fbase(bool masked) { masked_fbase = masked; }
...
int KinematicsSolver::tasks_count() { return tasks.size(); }

void KinematicsSolver::compute_limits_inequalities() {
  if (velocity_limits && dt == 0.) {
    throw std::runtime_error(
        "You enabled velocity limits but didn't set solver.dt");
  }
  ...
  if (velocity_limits) {
    problem.add_constraint(qd->expr(6) <=
                           dt * robot.model.velocityLimit.bottomRows(N - 6));
    problem.add_constraint(-dt * robot.model.velocityLimit.bottomRows(N - 6) <=
                           qd->expr(6));
  }
}
...
Eigen::VectorXd KinematicsSolver::solve(bool apply) {
  ...
  for (auto task : tasks) {
    ...
    // This could be written (task->A * qd->expr() == task->b), but would come
    // with the significant cost of multiplying A with identity matrix for each
    // task
    Expression e;
    e.A = task->A;
    ...
    if (task->priority == Task::Priority::Scaled) {
      if (!has_scaling) {
        ...
        // We introduce a scale factor 0 <= s <= 1 that we want to take as close
        // as possible to 1.0
        ...
        problem.add_constraint(scale_variable->expr() == 1)
            .configure(ProblemConstraint::Soft, 1.0);
      }
  ...
  if (apply) {
    ...
    if (dt > 0) {
      ...
      robot.state.qd =
          pinocchio::difference(robot.model, q_save, robot.state.q) / dt;
...
std::set<Task *> KinematicsSolver::get_tasks() { return tasks; }
...
void KinematicsSolver::dump_status_stream(std::ostream &stream) {
  ...
  for (auto task : tasks) {
    task->update();
    stream << "  * " << task->name << " [" << task->type_name() << "]"
           << std::endl;
    stream << "    - Priority: ";
...
void KinematicsSolver::dump_status() { dump_status_stream(std::cout); }
