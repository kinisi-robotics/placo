...
RobotWrapper::RobotWrapper(std::string model_directory, int flags,
  ...
  if (urdf_content != "") {
    if (flags & Flags::MJCF) {
      throw std::runtime_error(
          "Loading MJCF with direct contents is not supported");
    } else {
      pinocchio::urdf::buildModelFromXML(urdf_content, root_joint, model);
      std::istringstream stream(urdf_content);
      pinocchio::urdf::buildGeom(model, stream, pinocchio::COLLISION,
    ...
    if (flags & Flags::MJCF) {
      ...
      pinocchio::mjcf::buildGeom(model, urdf_filename, pinocchio::COLLISION,
                                 collision_model);
    } else {
  ...
  if (flags & COLLISION_AS_VISUAL) {
    ...
    if (urdf_content != "") {
      ...
      std::istringstream stream(urdf_content);
      pinocchio::urdf::buildGeom(model, stream, pinocchio::VISUAL, visual_model,
                                 model_directory);
    } else {
      if (flags & Flags::MJCF) {
        pinocchio::mjcf::buildGeom(model, urdf_filename, pinocchio::VISUAL,
                                   visual_model);
      } else {
  ...
  if (collisions.size() > 0) {
    std::cerr << "WARNING: Robot has the following self collisions in neutral "
                 "position:"
              << std::endl;

    for (auto &collision : collisions) {
      std::cerr << "  -" << collision.bodyA << " collides with "
                << collision.bodyB << std::endl;
    }
  }
}
...
void RobotWrapper::reset() { state = neutral_state(); }
...
void RobotWrapper::set_joint_acceleration(const std::string &name,
                                          double value) {
...
void RobotWrapper::set_joint_limits(const std::string &name, double lower,
                                    double upper) {
...
RobotWrapper::get_joint_limits(const std::string &name) {
  int k = get_joint_offset(name);
  return std::make_pair(model.lowerPositionLimit[k],
                        model.upperPositionLimit[k]);
}
...
Eigen::Affine3d RobotWrapper::get_T_world_fbase() {
  ...
  transformation.translation() =
      Eigen::Vector3d(state.q[0], state.q[1], state.q[2]);

  auto rotation =
      Eigen::Quaterniond(state.q[6], state.q[3], state.q[4], state.q[5]);
...
void RobotWrapper::update_kinematics() {
  ...
  pinocchio::computeJointJacobiansTimeVariation(model, *data, state.q,
                                                state.qd);
...
Eigen::MatrixXd RobotWrapper::get_frame_hessian(FrameIndex frame,
                                                int joint_v_index) {
  ...
  pinocchio::Tensor<double, 3, 0> H = pinocchio::getJointKinematicHessian(
      model, *data, joint_id, pinocchio::LOCAL);
  const Eigen::DenseIndex matrix_offset = 6 * model.nv;

  Eigen::Map<Eigen::MatrixXd> hessian(H.data() + joint_v_index * matrix_offset,
                                      6, model.nv);
...
void RobotWrapper::load_collision_pairs(const std::string &filename) {
  ...
  for (size_t k = 0; k < collision_model.geometryObjects.size(); k++) {
    if (collision_model.geometryObjects[k].parentFrame !=
        std::numeric_limits<FrameIndex>::max()) {
      std::string name =
          model.frames[collision_model.geometryObjects[k].parentFrame].name;
    ...
    if (entry.size() == 2) {
      if (entry[0].isInt() && entry[1].isInt()) {
        ...
        if (!name_to_objects.count(obj1) || !name_to_objects.count(obj2)) {
          std::ostringstream oss;
          oss << "Collision pair [" << obj1 << ", " << obj2
              << "] can't be loaded (check that bodies exists)";
          throw std::runtime_error(oss.str());
        }
        ...
        throw std::runtime_error(
            "Collision pairs should be array of int or strings");
      }
    }
  }
}
...
std::vector<RobotWrapper::Collision>
RobotWrapper::self_collisions(bool stop_at_first) {
  ...
  pinocchio::computeCollisions(model, *data, collision_model, geom_data,
                               state.q);
  ...
  for (size_t k = 0; k < collision_model.collisionPairs.size(); ++k) {
    ...
    if (cr.isCollision()) {
      ...
      collision.parentB =
          collision_model.geometryObjects[cp.second].parentJoint;
...
std::vector<RobotWrapper::Distance> RobotWrapper::distances() {
  ...
  // And test all the collision pairs
  pinocchio::computeDistances(model, *data, collision_model, geom_data,
                              state.q);
...
static pinocchio::ReferenceFrame
string_to_reference(const std::string &reference) {
  if (reference == "local_world_aligned") {
    ...
    oss << "Unknown reference: " << reference
        << ", use one of: world, local, local_world_aligned.";
    throw std::runtime_error(oss.str());
  }
}
...
Eigen::MatrixXd RobotWrapper::frame_jacobian(pinocchio::FrameIndex frame,
...
Eigen::MatrixXd
RobotWrapper::frame_jacobian_time_variation(const std::string &frame,
                                            const std::string &reference) {
  return frame_jacobian_time_variation(get_frame_index(frame),
                                       string_to_reference(reference));
}

Eigen::MatrixXd
RobotWrapper::frame_jacobian_time_variation(pinocchio::FrameIndex frame,
                                            pinocchio::ReferenceFrame ref) {
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
      jacobian(6, model.nv);
  jacobian.setZero();
...
Eigen::MatrixXd RobotWrapper::joint_jacobian(const std::string &joint,
                                             const std::string &reference) {
  return joint_jacobian(model.getJointId(joint),
                        string_to_reference(reference));
}

Eigen::MatrixXd RobotWrapper::joint_jacobian(pinocchio::JointIndex joint,
                                             pinocchio::ReferenceFrame ref) {
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
      jacobian(6, model.nv);
  jacobian.setZero();
...
Eigen::MatrixXd
RobotWrapper::joint_jacobian_time_variation(const std::string &joint,
                                            const std::string &reference) {
  return joint_jacobian(model.getJointId(joint),
                        string_to_reference(reference));
}
...
Eigen::MatrixXd
RobotWrapper::joint_jacobian_time_variation(pinocchio::JointIndex joint,
                                            pinocchio::ReferenceFrame ref) {
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
      jacobian(6, model.nv);
  jacobian.setZero();
...
Eigen::MatrixXd
RobotWrapper::relative_position_jacobian(pinocchio::FrameIndex frame_a,
                                         pinocchio::FrameIndex frame_b) {
  ...
  Eigen::MatrixXd J_a_pos =
      frame_jacobian(frame_a, pinocchio::LOCAL_WORLD_ALIGNED)
          .block(0, 0, 3, model.nv);
  Eigen::MatrixXd J_a_rot =
      frame_jacobian(frame_a, pinocchio::LOCAL_WORLD_ALIGNED)
          .block(3, 0, 3, model.nv);
  Eigen::MatrixXd J_b_pos =
      frame_jacobian(frame_b, pinocchio::LOCAL_WORLD_ALIGNED)
          .block(0, 0, 3, model.nv);

  return (R_world_a.transpose() * (J_b_pos - J_a_pos) +
          pinocchio::skew(T_a_b.translation()) * R_world_a.transpose() *
              J_a_rot);
}

Eigen::MatrixXd
RobotWrapper::relative_position_jacobian(const std::string &frame_a,
                                         const std::string &frame_b) {
  return relative_position_jacobian(get_frame_index(frame_a),
                                    get_frame_index(frame_b));
}
...
Eigen::Matrix3Xd RobotWrapper::com_jacobian_time_variation() {
  ...
  return pinocchio::computeCentroidalMapTimeVariation(model, *data, state.q,
                                                      state.qd)
             .topRows(3) /
         total_mass();
}
...
Eigen::MatrixXd RobotWrapper::mass_matrix() {
  ...
  // We account for inertia by adding the rotor inertia times the squared gear
  // ratio to the diagonal (see Featherstone, Rigid Body Dynamics Algorithm,
  // 2008, end of chapter 9.6)
  for (int k = 0; k < M.rows(); k++) {
    M(k, k) += model.rotorGearRatio[k] * model.rotorGearRatio[k] *
               model.rotorInertia[k];
  }
...
Eigen::VectorXd RobotWrapper::static_gravity_compensation_torques(
  ...
  // We know that no torque can be provided by floating base. We can then
  // compute f_ext using the unactuated part of the jacobian matrix
...
Eigen::VectorXd
RobotWrapper::static_gravity_compensation_torques(std::string frame) {
  return static_gravity_compensation_torques(get_frame_index(frame));
}

Eigen::VectorXd RobotWrapper::torques_from_acceleration_with_fixed_frame(
    Eigen::VectorXd qdd_a, RobotWrapper::FrameIndex frameIndex) {
  ...
  Eigen::MatrixXd J = frame_jacobian(frameIndex);
  Eigen::VectorXd f =
      J.transpose().block(0, 0, 6, 6).inverse() * h.block(0, 0, 6, 1);
...
Eigen::VectorXd
RobotWrapper::torques_from_acceleration_with_fixed_frame(Eigen::VectorXd qdd_a,
                                                         std::string frame) {
  return torques_from_acceleration_with_fixed_frame(qdd_a,
...
std::vector<std::string> RobotWrapper::joint_names(bool include_floating_base) {
  ...
  if (!include_floating_base) {
    joints.erase(std::remove(joints.begin(), joints.end(), "universe"),
                 joints.end());
    joints.erase(std::remove(joints.begin(), joints.end(), "root_joint"),
                 joints.end());
  }
  return joints;
}
...
void RobotWrapper::add_q_noise(double noise) {
  ...
  // Adding some noise in direction of a random configuration (except floating
  // base)
