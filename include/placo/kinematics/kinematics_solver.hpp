...
// Tasks
#include "placo/kinematics/axis_align_task.hpp"
#include "placo/kinematics/centroidal_momentum_task.hpp"
#include "placo/kinematics/com_task.hpp"
#include "placo/kinematics/distance_task.hpp"
#include "placo/kinematics/frame_task.hpp"
#include "placo/kinematics/gear_task.hpp"
#include "placo/kinematics/joints_task.hpp"
#include "placo/kinematics/kinetic_energy_regularization_task.hpp"
#include "placo/kinematics/manipulability_task.hpp"
#include "placo/kinematics/orientation_task.hpp"
#include "placo/kinematics/position_task.hpp"
#include "placo/kinematics/regularization_task.hpp"
#include "placo/kinematics/relative_frame_task.hpp"
#include "placo/kinematics/relative_orientation_task.hpp"
#include "placo/kinematics/relative_position_task.hpp"
#include "placo/kinematics/task.hpp"
#include "placo/kinematics/wheel_task.hpp"

// Constraints
#include "placo/kinematics/avoid_self_collisions_constraint.hpp"
#include "placo/kinematics/com_polygon_constraint.hpp"
#include "placo/kinematics/cone_constraint.hpp"
#include "placo/kinematics/constraint.hpp"
#include "placo/kinematics/distance_constraint.hpp"
#include "placo/kinematics/joint_space_half_spaces_constraint.hpp"
#include "placo/kinematics/yaw_constraint.hpp"
...
public:
   ...
   * @pyignore
   */
  PositionTask &add_position_task(model::RobotWrapper::FrameIndex frame,
   ...
   * @return position task
   */
  PositionTask &add_position_task(std::string frame,
                                  Eigen::Vector3d target_world);
  ...
  add_relative_position_task(model::RobotWrapper::FrameIndex frame_a,
   ...
   * @return relative position task
   */
  RelativePositionTask &add_relative_position_task(std::string frame_a,
                                                   std::string frame_b,
                                                   Eigen::Vector3d target);
   ...
   * @param frame the robot frame we want to control
   * @param R_world_frame the target orientation we want to achieve, expressed
   * in the world (as T_world_frame)
   * @return orientation task
   * @pyignore
   */
  OrientationTask &add_orientation_task(model::RobotWrapper::FrameIndex frame,
   ...
   * @return orientation task
   */
  OrientationTask &add_orientation_task(std::string frame,
                                        Eigen::Matrix3d R_world_frame);
  ...
  add_relative_orientation_task(model::RobotWrapper::FrameIndex frame_a,
                                model::RobotWrapper::FrameIndex frame_b,
   ...
   * @return relative orientation task
   */
  RelativeOrientationTask &add_relative_orientation_task(std::string frame_a,
                                                         std::string frame_b,
                                                         Eigen::Matrix3d R_a_b);

  /**
   * @brief Adds a frame task, this is equivalent to a position + orientation
   * task, resulting in decoupled tasks for a given frame
   * @param frame the robot frame we want to control
   * @param T_world_frame the target for the frame we want to control, expressed
   * in the world (as T_world_frame)
   ...
   */
  FrameTask
  add_frame_task(model::RobotWrapper::FrameIndex frame,
   ...
   */
  FrameTask
  add_frame_task(std::string frame,
                 Eigen::Affine3d T_world_frame = Eigen::Affine3d::Identity());
   ...
   */
  RelativeFrameTask
  add_relative_frame_task(model::RobotWrapper::FrameIndex frame_a,
                          model::RobotWrapper::FrameIndex frame_b,
   ...
   * @return relative frame task
   */
  RelativeFrameTask add_relative_frame_task(std::string frame_a,
                                            std::string frame_b,
                                            Eigen::Affine3d T_a_b);
   ...
   */
  WheelTask &add_wheel_task(const std::string joint, double radius,
   ...
   * @pyignore
   */
  DistanceTask &add_distance_task(model::RobotWrapper::FrameIndex frame_a,
                                  model::RobotWrapper::FrameIndex frame_b,
                                  double distance);
   ...
   * @return distance task
   */
  DistanceTask &add_distance_task(std::string frame_a, std::string frame_b,
                                  double distance);

  /**
   * @brief Adds an axis alignment task. The goal here is to keep the given axis
   * (expressed in the given frame) aligned with another one (given in the
   * world)
   * @param frame the robot frame we want to control
   * @param axis_frame the axis to align, expressed in the robot frame
   * @param targetAxis_world the target axis (in the world) we want to be
   * aligned with
   */
  AxisAlignTask &add_axisalign_task(model::RobotWrapper::FrameIndex frame,
   ...
   * aligned with
   */
  AxisAlignTask &add_axisalign_task(std::string frame,
                                    Eigen::Vector3d axis_frame,
                                    Eigen::Vector3d targetAxis_world);
   ...
   */
  ManipulabilityTask &
  add_manipulability_task(model::RobotWrapper::FrameIndex frame,
                          ManipulabilityTask::Type type, double lambda = 1.0);
   ...
   * @return manipulability task
   */
  ManipulabilityTask &add_manipulability_task(std::string frame,
  ...
  add_kinetic_energy_regularization_task(double magnitude = 1e-6);
  ...
  add_com_polygon_constraint(std::vector<Eigen::Vector2d> polygon,
                             double margin = 0.);
  ...
  add_joint_space_half_spaces_constraint(Eigen::MatrixXd A, Eigen::VectorXd b);

  /**
   * @brief Adds a Cone constraint
   * @param frame_a frame A
   * @param frame_b frame B
   * @param alpha_max alpha max (in radians) between the frame z-axis and the
   * cone frame zt-axis
   * @return constraint
   * @pyignore
   */
  ConeConstraint &add_cone_constraint(model::RobotWrapper::FrameIndex frame_a,
                                      model::RobotWrapper::FrameIndex frame_b,
                                      double alpha_max);
   ...
   * @return constraint
   */
  ConeConstraint &add_cone_constraint(std::string frame_a, std::string frame_b,
                                      double alpha_max);
   ...
   * @pyignore
   */
  YawConstraint &add_yaw_constraint(model::RobotWrapper::FrameIndex frame_a,
                                    model::RobotWrapper::FrameIndex frame_b,
                                    double alpha_max);
   ...
   * @return constraint
   */
  YawConstraint &add_yaw_constraint(std::string frame_a, std::string frame_b,
                                    double alpha_max);
   ...
   */
  DistanceConstraint &
  add_distance_constraint(model::RobotWrapper::FrameIndex frame_a,
                          model::RobotWrapper::FrameIndex frame_b,
                          double distance_max);
   ...
   * @return constraint
   */
  DistanceConstraint &add_distance_constraint(std::string frame_a,
                                              std::string frame_b,
                                              double distance_max);
  ...
  /**
   * @brief Masks (disables a DoF) from being used by the QP solver (it can't
   * provide speed)
  ...
  /**
   * @brief Unmsks (enables a DoF) from being used by the QP solver (it can
   * provide speed)
   * @param dof the dof name
   */
  ...
  template <typename T> T &add_task(T *task) {
    task_id += 1;
    task->solver = this;
  ...
  template <typename T> T &add_constraint(T *constraint) {
    constraint_id += 1;
...
protected:
  ...
  // Task id (this is only useful when task names are not specified, each task
  // will have an unique ID)
  int task_id = 0;
  int constraint_id = 0;
};
} // namespace placo::kinematics