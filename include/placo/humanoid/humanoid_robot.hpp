...
#include "rhoban_utils/history/history.hpp"
#endif
...
public:
  ...
  /**
   * @brief Compute the center of mass velocity from the speed of the motors and
   * the orientation of the trunk
   ...
   * @param contact_forces Contact forces from the feet (forces are supposed
   * normal to the ground)
   * @param use_non_linear_effects If true, non linear effects are taken into
   * account (state.qd necessary)
   * @return Torques of the motors
   */
  Eigen::VectorXd get_torques(Eigen::VectorXd acc_a,
   ...
   * @param P_world_target Position in the world referential of the target we
   * want to look at
   * @return True if a solution is found, false if not
   */
  bool camera_look_at(double &pan, double &tilt,
   ...
   * @param qd_joints If not empty, use these values for the joint velocities
   * (state.qd) (NOT TESTED)
   */
  void
  read_from_histories(rhoban_utils::HistoryCollection &histories,
                      double timestamp, std::string source = "read",
                      bool use_imu = false,
                      Eigen::VectorXd qd_joints = Eigen::VectorXd::Zero(1));
#endif
  ...
  double dist_z_pan_tilt;   // Distance along z between the pan DoF and the tilt
                            // DoF in the head
  double dist_z_pan_camera; // Distance along z between the pan DoF and the
                            // camera in the head
  double dist_y_trunk_foot; // Distance along y between the trunk and the
                            // left_foot frame in model
};
} // namespace placo::humanoid