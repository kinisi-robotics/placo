...
bool Expression::is_scalar() const { return rows() == 1; }

bool Expression::is_constant() const { return cols() == 0; }
...
int Expression::cols() const { return A.cols(); }

int Expression::rows() const { return A.rows(); }
...
Expression Expression::operator+(const Expression &other) const {
  ...
  if (rows() != other.rows()) {
    ...
    oss << "Trying to add expressions with different # of rows (" << rows()
        << " vs " << other.rows() << ")";
  ...
  e.A.block(0, 0, rows(), other.cols()) +=
      other.A.block(0, 0, rows(), other.cols());
  e.b = b;
  e.b += other.b;
...
Expression operator*(double f, const Expression &e) { return e * f; }
...
Expression Expression::operator*(const Expression &other) const {
  if (is_scalar() && other.is_constant()) {
    ...
    throw std::runtime_error("Two expression can only be multiplied if one is "
                             "scalar and the other constant");
  }
}
...
ProblemConstraint operator==(double f, const Expression &e) { return e == f; }
}; // namespace placo::problem